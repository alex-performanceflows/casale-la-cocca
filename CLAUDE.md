...
## 11. Stato attuale (v1 — sessione giugno 2025)

> Aggiornato al 25/06/2025. Da mantenere aggiornato ad ogni sessione di lavoro.

### Stack e configurazione
- **Build**: `npx tailwindcss -i ./assets/css/input.css -o ./assets/css/styles.css --minify`
- **Dev server**: `python3 -m http.server 3456` (porta 3456)
- **Vercel**: `vercel.json` presente per deploy statico

### Foto
- Le foto professionali del fotografo sono in `Casale la Cocca - Foto/` (36 JPG originali, non toccare)
- Copiate e rinominate semanticamente in `assets/img/*.jpg` (28 file)
- `assets/img/placeholders/` contiene le vecchie webp da Puglia Paradise — mantenute per backup
- **Hero**: `assets/img/hero-villa-pool-sunset.jpg` (DSC_6001ce — tramonto piscina, foto principale)
- **Logo**: `assets/img/logo.png` (500×500 RGB, nessuna trasparenza — richiedere versione con alpha o SVG)

### Decisioni di design prese
- **Font**: Fraunces (serif display) + Inter (sans) via Google Fonts — da self-hostare in produzione
- **Palette**: cream `#F7F4EE` / carbon `#2B2723` / olive `#5E6B4F` / clay `#B08463` / sand `#D8CFBF`
- **Border radius**: `rounded-sm` ovunque (coerenza) — i form field usavano `rounded-lg`, corretto
- **Logo nell'hero**: circolare (`rounded-full`), decorativo (`aria-hidden="true"`, `alt=""`), dimensione `w-36→w-52` responsive
- **Logo nel nav**: piccolo `h-10 w-10` affiancato al testo "Casale La Cocca"
- **Separatori full-bleed**: due sezioni senza testo tra le sezioni principali (piscina aerea + villa drone)
- **Pull quote**: sezione tipografica tra Comfort e Posizione — "Il lusso vero non fa rumore. Fa luce."
- **Striscia piattaforme**: sotto l'hero, trust signal immediato (Puglia Paradise, Airbnb, Hotels Apulia, Agoda)

### Decisioni di CRO e copy
- **CTA principale**: "Controlla disponibilità" (più diretto di "Richiedi disponibilità")
- **Hero subtitle**: benefici concreti e sensoriali, non solo descrittivi
- **Contact title**: "Hai le date in testa?" — conversazionale, abbassa la resistenza
- **Pool framing**: "non ha un bordo — ha un orizzonte"
- **Form**: check-in con `min=oggi` via JS; checkout si aggiorna dinamicamente
- **Date minime**: gestite in `<script>` inline in fondo a index.html

### Bug corretti
- JSON-LD aveva un commento JS (`/* TODO_GEO */`) che invalidava i dati strutturati → rimosso
- `<button data-gallery-item>` mancava di `type="button"` → aggiunto
- `cursor-pointer` mancante su bottoni galleria e toggle lingua → aggiunto
- Skip link per accessibilità da tastiera → aggiunto
- `alt` logo nell'hero ridondante con `<h1>` → corretto (`alt=""` + `aria-hidden="true"`)

### TODO ancora aperti (proprietario)
- `TODO_DOMINIO` — URL canonico definitivo
- `TODO_EMAIL_PROPRIETARIO` + `TODO_FORM_ENDPOINT` (Web3Forms o Formspree)
- `TODO_TELEFONO` / `TODO_WHATSAPP`
- `TODO_NOME_FOTOGRAFO` — credito in footer
- `TODO_PREZZI` — politica prezzi da inserire o collegare
- `TODO_PAGINE_LEGALI` — privacy policy e cookie policy GDPR
- `TODO_GEO` — coordinate reali lat/lng per JSON-LD e mappa OSM
- Logo senza trasparenza — richiedere PNG con alpha o SVG al grafico
- Testimonianze reali da aggiungere (struttura placeholder pronta)
- `srcset` e formati multipli per le foto quando arriveranno i file ad alta risoluzione finali
- Self-hosting font Google Fonts (per performance e privacy)
